...
@SpringJUnitConfig(locations = "classpath:/com/dummy/myerp/model/applicationContext.xml")
public abstract class AbstractConsumerIt {
	...
	private static void initMapEcritureExpected() {
		mapEcritureExpected.put(-1, new EcritureComptable(-1, mapJournalExpected.get("AC"), "AC-2016/00001",
				DateHelper.getDate(31, 12, 2016), "Cartouches d’imprimante", getListeLigneEcriture(-1)));
		mapEcritureExpected.put(-2, new EcritureComptable(-2, mapJournalExpected.get("VE"), "VE-2016/00002",
				DateHelper.getDate(30, 12, 2016), "TMA Appli Xxx", getListeLigneEcriture(-2)));
		mapEcritureExpected.put(-3, new EcritureComptable(-3, mapJournalExpected.get("BQ"), "BQ-2016/00003",
				DateHelper.getDate(29, 12, 2016), "Paiement Facture F110001", getListeLigneEcriture(-3)));
		mapEcritureExpected.put(-4, new EcritureComptable(-4, mapJournalExpected.get("VE"), "VE-2016/00004",
				DateHelper.getDate(28, 12, 2016), "TMA Appli Yyy", getListeLigneEcriture(-4)));
		mapEcritureExpected.put(-5, new EcritureComptable(-5, mapJournalExpected.get("BQ"), "BQ-2016/00005",
				DateHelper.getDate(27, 12, 2016), "Paiement Facture C110002", getListeLigneEcriture(-5)));
	...
	protected static List<LigneEcritureComptable> getListeLigneEcriture(Integer ecritureId) {
		return mapLigneEcritureExpected.entrySet().stream()
				.filter(entry -> entry.getKey().toList().get(0).equals(ecritureId))
				.map(entry -> entry.getValue().deepCopy()).collect(Collectors.toList());
	}
